...
void compatibility_check(struct pc you_pc) //проверка совместимости комплектующих
{
	...
	(you_pc.cpu.power + you_pc.gpu.power) < you_pc.psu.power ? printf("Мощности БП хватает (в запасе примерно) %i ватт", you_pc.psu.power - (you_pc.cpu.power + you_pc.gpu.power)) : printf("Слабый БП, не хватает %i ватт", (you_pc.cpu.power + you_pc.gpu.power) - you_pc.psu.power);
}
...
void recom(struct pc you_pc)
{
	...
	printf("Ваша сборка сбалонсирована");
	return;
}
